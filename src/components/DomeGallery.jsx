...
function buildItems(pool, seg) {
  ...
  const evenYs = [];
  const oddYs = [];
  // Populate Y from pole to pole (approx -16 to 16 for a 35-segment sphere)
...
export default function DomeGallery({
  ...
  // Add a ref to track auto-rotate vertical direction bouncing
  const autoRotateDirX = useRef(1);

  useEffect(() => {
    if (!autoRotate) return;
    const step = () => {
      if (!isHoveredRef.current && !draggingRef.current && !inertiaRAF.current && !openingRef.current && rootRef.current?.getAttribute('data-enlarging') !== 'true') {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeed);
        
        let nextX = rotationRef.current.x + (autoRotateSpeedX * autoRotateDirX.current);
        if (nextX >= maxVerticalRotationDeg) {
          nextX = maxVerticalRotationDeg;
          autoRotateDirX.current = -1;
        } else if (nextX <= -maxVerticalRotationDeg) {
          nextX = -maxVerticalRotationDeg;
          autoRotateDirX.current = 1;
        }
  ...
  }, [autoRotate, autoRotateSpeed, autoRotateSpeedX, maxVerticalRotationDeg]);
